...
def loss_ridge(X, y, B, lmbda):
    return -np.dot(np.transpose(X), y - np.dot(X, B)) + lmbda*np.dot(np.transpose(B),B)
...
def minimize(X, y, loss_gradient,
    ...
    i = 0
    hell = np.zeros(B.shape)
    while i<max_iter:
        gradient_loss = loss_gradient(X, y, B, lmbda)
        if np.sum(np.dot(gradient_loss.T,gradient_loss))<precision:
            break
        hell = hell + gradient_loss**2
        B = B - np.multiply(eta/np.sqrt(np.sum(hell,axis = 1) + eps).reshape(-1,1),gradient_loss)     
        i = i + 1
    return B
...
class RidgeRegression621: # MODIFY THIS ONE
    ...
    def fit(self, X, y):
        ...
        self.B = minimize(X, y,
                          ...
                          self.lmbda,
                          self.max_iter,False)
        self.B = np.insert(self.B,0,y.mean(), axis=None)
