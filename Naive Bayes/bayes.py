...
def load_docs(docs_dirname:str) -> Sequence[Sequence]:
    ...
    """
    docs = filelist(docs_dirname)
    docs = [words(get_text(doc)) for doc in docs if '.txt' in doc]
    # YOUR CODE HERE
    return docs
...
def vocab(neg:Sequence[Sequence], pos:Sequence[Sequence]) -> dict:
    ...
    and so |V| is 3
    """
    neg_list = np.array(np.concatenate(neg))
    pos_list = np.array(np.concatenate(pos))
    final_list = np.unique(np.concatenate((neg_list,pos_list)))
    final_list = np.insert(final_list,0,'__unknown__')
    V = defaultintdict()
    for x,y in zip(final_list,np.arange(len(final_list))):
        V[x] = y
...
def vectorize(V:dict, docwords:Sequence) -> np.ndarray:
    ...
    index_vector = [V[x] if x in V.keys() else 0 for x in docwords]
    vector = np.zeros(len(V.keys()))
...
class NaiveBayes621:
    ...
    def fit(self, X:np.ndarray, y:np.ndarray) -> None:
        ...
        """
        # YOUR CODE HERE
        self.prior =  np.array([y.sum()/len(y),1 - y.sum()/len(y)])
        word_one = X[np.where(y==1)]
        word_zero = X[np.where(y==0)]
        prob_one_vector = np.add(word_one.sum(axis = 0),1)/(word_one.sum() + X.shape[1] + 1)
        prob_zero_vector = np.add(word_zero.sum(axis = 0),1)/(word_zero.sum() + X.shape[1] + 1)
        self.likelihood = np.log(np.array([prob_one_vector,prob_zero_vector]))
    ...
    def predict(self, X:np.ndarray) -> np.ndarray:
        ...
        """
        final_vector = np.dot(X,self.likelihood.T) + np.log(self.prior)
        return np.where(final_vector[:,0]<final_vector[:,1],0,1)
        # YOUR CODE HERE
...
def kfold_CV(model, X:np.ndarray, y:np.ndarray, k=4) -> np.ndarray:
    ...
    """
    kf = KFold(n_splits = 4,random_state = 999,shuffle = True) #fill this in# ) # use same split every time for student eval
    accuracies = []
