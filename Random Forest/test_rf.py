...
def run_classification_test(X, y, ntrials=1, min_samples_leaf=3, max_features=0.3, min_training_score=1.0, grace=.07, oob=False, n_estimators=15):
    ...
    assert np.mean(train_scores, dtype=np.float64) >= min_training_score, \
           f"Training accuracy: {np.mean(train_scores):.2f} must be >= {min_training_score:.2f}"
    assert np.mean(test_scores, dtype=np.float64)+grace >= np.mean(sklearn_test_scores), \
